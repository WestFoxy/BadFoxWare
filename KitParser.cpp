...
bool Compare_Skin(Kit_t &a, Kit_t &b)
{
	return a.id < b.id;
...
void InitializeKits()
{
	...
	auto sig_address = Utils_fox.FindPatternMask("client_panorama.dll", (BYTE*)"\xE8\x00\x00\x00\x00\xFF\x76\x0C\x8D\x48\x04\xE8", "x????xxxxxxx");
	...
	// Dump paint kits
	{
		...
		std::sort(k_skins.begin(), k_skins.end()/*, Compare_Skin*/);
		std::sort(k_gloves.begin(), k_gloves.end()/*, Compare_Skin*/);
	}

	// Dump sticker kits
	/*
		auto sticker_sig = Utils_fox.FindPatternMask("client_panorama.dll", (BYTE*)"\x53\x8D\x48\x04\xE8\x00\x00\x00\x00\x8B\x4D\x10", "xxxxx????xxx") + 4;
