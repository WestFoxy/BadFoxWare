...
void ImGui::Render()
{
	...
	//ImGui::Image(g_Options.tImage, ImVec2(766, 400));
    ...
    if (first_begin_of_the_frame)
    {
        ...
        else
        {
            ...
            if (!(flags & ImGuiWindowFlags_NoTitleBar))
				...
				window->DrawList->AddRectFilledMultiColor(title_bar_rect.Min, title_bar_rect.Max, GetColorU32(ImGuiCol_TitleBg), GetColorU32(ImGuiCol_TitleBg), GetColorU32(ImGuiCol_TitleBgActive), GetColorU32(ImGuiCol_TitleBgActive));
            // Menu bar
...
bool ImGui::ButtonGradientEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	auto color_grey = ImGui::GetColorU32(ImVec4(0.95f, 0, 0, 0.2));
	auto color_black = ImGui::GetColorU32(ImVec4(0.75f, 0, 0, 1.0));
...
bool ImGui::ButtonGradient(const char* label, const ImVec2& size_arg)
{
	return ButtonGradientEx(label, size_arg, 0);
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImRect check_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(label_size.y + style.FramePadding.y*2, label_size.y + style.FramePadding.y*1.5)); // We want a square shape to we use Y twice
...
bool ImGui::ColorEdit4Label(float col[4], ImGuiColorEditFlags flags)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
