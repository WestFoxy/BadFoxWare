...
void CCSGrenadeHint::Paint()
{
	...
	if ((type) && path.size()>1 && act != ACT_NONE && weapon->IsGrenade())
	{
		...
		{
			CCSGrenadeHint::Draw3dCube(2.f, it->second, it->first, Color(0, 255, 0, 200));
		}

		CCSGrenadeHint::Draw3dCube(2.f, OtherCollisions.rbegin()->second, OtherCollisions.rbegin()->first, Color(255, 0, 0, 200));
...
void CCSGrenadeHint::Draw3dCube(float scalar, QAngle angles, Vector middle_origin, Color outline)
{
