...
{
	if (!local || !g_Options.enable_legitbot)
		return;
...
void legitbot::weapon_settings(C_BaseCombatWeapon* weapon)
{
	...
	if (weapon->IsPistol())
	{
		...
		hitbox = g_Options.ForceBody ? 4 : g_Options.hitbox_pistols;
	...
	{
		hitbox = g_Options.ForceBody ? 4 : g_Options.hitbox_rifles;
		aim_key = g_Options.aimkey;
	...
	{
		hitbox = g_Options.ForceBody ? 4 : g_Options.hitbox_snipers;
		aim_key = g_Options.aimkey;
