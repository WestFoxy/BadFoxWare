...
static bool IsKeyPressedMap1(ImGuiKey key, bool repeat = true)
{
...
bool ImGui::Hotkey(const char* label, int* k, const ImVec2& size_arg)
{
	...
	if (g.ActiveId == id) {
		...
		if (IsKeyPressedMap1(ImGuiKey_Escape)) {
			*k = 0;
			ImGui::ClearActiveID();
		}
		else {
	...
	ImGui::RenderFrame(frame_bb.Min, frame_bb.Max, ImGui::GetColorU32(ImVec4(0.20f, 0.20f, 0.20f, 1.0f)), true, style.FrameRounding);
