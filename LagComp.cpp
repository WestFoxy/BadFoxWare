...
std::deque<TimeWarp::SequenceData>twsequences;
...
void TimeWarp::AddLatencyToNetchan(INetChannel *netchan, float Latency)
{
...
bool TimeWarp::IsTickValid(int tick)
{
	...
	float latecy = g_Options.Fakelat;
	float ping = g_Options.fakelatency_amt ? latecy : 0.2f;
...
void TimeWarp::UpdateIncomingSequences() {
	...
	if (ClientState)
	{
		...
		if (netchan) {
			if (netchan->m_nInSequenceNr > lastincomingsequencenumber) {
				...
				twsequences.push_front (TimeWarp::SequenceData(netchan->m_nInReliableState, netchan->m_nOutReliableState, netchan->m_nInSequenceNr, g_GlobalVars->realtime));
			}
...
void TimeWarp::StoreRecords(CUserCmd* cmd, C_BasePlayer* local)
{
	g_Options.mutex_validrecords_lock = true;
	for (int i = 1; i <= 64; i++)
	{
	...
	g_Options.mutex_validrecords_lock = false;
}

void TimeWarp::DoBackTrack(CUserCmd* cmd, C_BasePlayer* pLocal)
{
	...
	if (cmd->buttons & IN_ATTACK || cmd->buttons & IN_ATTACK2)
		if (bestFov < bestFovPlayer && bestRecord && g_Options.misc_backtrack)
			cmd->tick_count = TIME_TO_TICKS(bestRecord->simtime + GetLerpTime());
}
void TimeWarp::ClearIncomingSequences()
{
	twsequences.clear();
}

TimeWarp* backtracking = new TimeWarp();
