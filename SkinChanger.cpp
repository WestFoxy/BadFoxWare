...
void ForceItemUpdate(C_WeaponCSBase * item)
{
	...
	C_EconItemViewBT& view = item->m_AttributeManager.m_Item;
...
static void ApplyConfigOnAttributableItem(C_BaseAttributableItem * item, const EconomyItem_t * config, unsigned xuid_low)
{
	...
	{
		auto world_model_handle = item->m_hWeaponWorldModelATITEM();
...
{
	long res = SHGetFolderPath(NULL, CSIDL_PERSONAL, NULL, SHGFP_TYPE_CURRENT, g_Options.my_documents_folder);
	if (res == S_OK)
	{
		std::string fPath = std::string(g_Options.my_documents_folder) + "\\L1SA\\" + "skinchanger.json";
