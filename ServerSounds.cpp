...
void ServerSound::AdjustPlayerFinish()
{
	...
	for (auto& RestorePlayer : m_arRestorePlayers)
	{
		...
		*player->GetIsDormantPt() = RestorePlayer.m_bDormant;
	}
...
void ServerSound::AdjustPlayerBegin(C_BasePlayer * player)
{
	...
	constexpr int EXPIRE_DURATION = 3000; // miliseconds-ish?
	...
	if (sound_expired && alpha_ent[player->EntIndex()] == 0.f)
		return;
	...
	current_player.m_vecAbsOrigin = player->m_vecABSorigin();
